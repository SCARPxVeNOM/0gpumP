...
class DataService {
  ...
  async updateUserProfile(walletAddress, profileData) {
    ...
    const profileHash = profileData.profileHash || null;
  ...
  async addTradingTransaction(transactionData) {
    ...
    await db.run(
      `INSERT INTO trading_history (
        coinId, txHash, blockNumber, timestamp, type,
        amount, price, volume, gasUsed, gasPrice
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        transactionData.coinId, transactionData.txHash,
        transactionData.blockNumber, transactionData.timestamp, transactionData.type,
        transactionData.amount, transactionData.price, transactionData.volume,
        transactionData.gasUsed, transactionData.gasPrice
      ]
    );
  ...
  /**
   * Get user trading history (Note: This would need to be implemented differently
   * since trading_history doesn't have userAddress. This is a placeholder.)
   */
  async getUserTradingHistory(userAddress, limit = 50, offset = 0) {
    await this.initialize();
    
    // For now, return empty array since trading_history doesn't have userAddress
    // In a real implementation, you'd need to track user addresses in trading history
    console.warn('getUserTradingHistory: trading_history table does not have userAddress column');
    return [];
  }
